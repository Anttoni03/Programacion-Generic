...
public class Palabra {
    ...
    //método procedimiento codificar que encripta los carácteres alfabéticos de
    //la palabra siguiendo una codificación
    public void codificar()
    {
        final char [] ALFABETO = "abcdefghijklmnopqrstuvwxyz".toCharArray();
        final char [] CODIFICACION = "defghijklmnopqrstuvwxyzabc".toCharArray();
        
        for (int i = 0;i<numeroCaracteres; i++)
        {
            boolean esAlfabetico = true;
            int indice = 0;
            while (caracteres[i] != ALFABETO[indice] && esAlfabetico)
            {
                indice++;
            ...
            if (esAlfabetico)
                caracteres[i] = CODIFICACION[indice];
        }
    }
    
    //método procedimiento decodificar que desencripta los carácteres alfabéticos
    //de la palabra siguiendo una codificación
    public void decodificar()
    {
        ...
        for (int i = 0;i<numeroCaracteres; i++)
        {
            boolean esAlfabetico = true;
            int indice = 0;
            while (caracteres[i] != CODIFICACION[indice] && esAlfabetico)
            {
                indice++;
            
                if (indice >= CODIFICACION.length)
                    esAlfabetico = false;
            }
